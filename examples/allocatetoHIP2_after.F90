...
USE MODE_MNH_HIPFORT
    ...
    if (gfirst_call_zt1d_discretisation) then
       ...
       CALL MNH_HIPallocate(zt1d_discretisation,zt1d_discretisation_size)
...
USE MODE_MNH_HIPFORT
    ...
    if (LUseT) then
       CALL MNH_HIPallocate(r%st,1-halo_size,nlocalx+halo_size, &
...
USE MODE_MNH_HIPFORT
    ...
    if (LUseT) then
    CALL MNH_HIPallocate(r%st,1-halo_size,nlocal+halo_size,   &
                  1-halo_size,nlocal+halo_size,   &
                  0,nz+1)
    !$acc enter data create (r%st)
    endif
...
!!$          !$acc enter data create(ut0)          
       ...
       ! Create residual vector       
       CALL MNH_HIPallocate(Tjacobi(level)%r,nz)
       r => Tjacobi(level)%r
       !$acc enter data create(r)
       ! Allocate memory for auxiliary vectors for Thomas algorithm      
       CALL MNH_HIPallocate(Tjacobi(level)%c,nz)
       c => Tjacobi(level)%c
       !$acc enter data create(c)
       CALL MNH_HIPallocate(Tjacobi(level)%utmp,nz)
...
!!$    integer :: ii,ij
    ...
    if (LUseT ) then
      ...
      if ( Lfirst_call_level_tridiag_mnhallT(level) ) then
         ...
         CALL MNH_HIPallocate(Ttridiag_mnh(level)%Tij)
         CALL MNH_HIPallocate(Ttridiag_mnh(level)%alpha_div_Tij)
