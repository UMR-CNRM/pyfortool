...
!               ------------
!
USE MODD_ACLIB_n, ONLY: YAEROSOLS
...
USE MODD_CH_AEROSOL
USE MODD_CH_ALT_EMIS
USE MODD_CH_MNHC_n, ONLY : LUSECHEM, LCH_CONV_SCAV, LCH_CONV_LINOX, LALT_EMIS
USE MODD_CLOUD_MF_n
USE MODD_CONDSAMP
USE MODD_CONF
USE MODD_CONF_n
...
!   
  ...
  END IF
!
! If RADIATION call, check clouds in parallel
!
IF (GRAD) THEN
!
...
!
    CASE('ECMW' , 'ECRA')
      ...
      XDTHRADSW(:,:,:)=0.0
      XDTHRADLW(:,:,:)=0.0
      CALL RADIATIONS_AGG(YLDIMPHYEX, NRAD_AGG,NI_RAD_AGG,NJ_RAD_AGG,NIOR_RAD_AGG,NJOR_RAD_AGG, NRAD_AGG_FLAG, TPFILE,&
...
!
  ...
  IF ( LCHTRANS ) THEN  ! update tracers for chemical transport
    ...
    ELSE
      ...
      ENDDO
    ENDIF
      CALL PPP2DSLT(YDCST_ACLIB,YAEROSOLS, NMODE_DST, NSV_DSTEND-NSV_DSTBEG+1, &
              ZSVDST(IIB:IIE,IJB:IJE,IKB:IKE,:), XRHODREF(IIB:IIE,IJB:IJE,IKB:IKE),&
    ...
    ELSE
      ...
      ENDDO
    END IF
      CALL PPP2DSLT(YDCST_ACLIB,YAEROSOLS, NMODE_SLT, NSV_SLTEND-NSV_SLTBEG+1, &
              ZSVSLT(IIB:IIE,IJB:IJE,IKB:IKE,:), XRHODREF(IIB:IIE,IJB:IJE,IKB:IKE),&
...
!
  IF ((LDUST).AND.(LCHTRANS)) THEN ! dust convective balance
    ...
    ELSE
      ...
      ENDDO
    ENDIF
      CALL DSLT2PPP(YDCST_ACLIB,YAEROSOLS, NMODE_DST, NSV_DSTEND-NSV_DSTBEG+1, &
  ...
  IF ((LSALT).AND.(LCHTRANS)) THEN ! sea salt convective balance
    ...
    ELSE
      ...
      ENDDO
    END IF
    CALL DSLT2PPP(YDCST_ACLIB,YAEROSOLS, NMODE_SLT, NSV_SLTEND-NSV_SLTBEG+1, &
...
!    IKIDM = JKID-IMI
 ...
 END IF
   CALL GROUND_PARAM_n(YLDIMPHYEX,ZSFTH, ZSFTH_WALL, ZSFTH_ROOF, ZCD_ROOF, ZSFRV, ZSFRV_WALL, ZSFRV_ROOF, &
                      ZSFSV, ZSFCO2, ZSFU, ZSFV, XSEA_UCU,XSEA_VCU,                                       &
...
ZTIME1 = ZTIME2
!
IF (LPASPOL .AND. IMI == NMODEL_PP) CALL PASPOL(XTSTEP, ZSFSV, ILUOUT, NVERB, TPFILE)
!
!
...
!*        4c.  EMISSION OF POLLUTANTS AT ALTITUDE
IF (LALT_EMIS) CALL CH_ALT_EMIS(XTSTEP, ILUOUT, NVERB )
!-----------------------------------------------------------------------------
!
!*        5a.    Drag force 
!               ----------
...
ELSE
  ALLOCATE(ZHGRADLEO(0,0,0,0))
END IF
!
IGRADIENTSGOG=4
IF(LGOGER) THEN
  ALLOCATE(ZHGRADGOG(IIU,IJU,IKU,IGRADIENTSGOG))
  ZHGRADGOG(:,:,:,1) =  GX_U_M(XUT(:,:,:), XDXX,XDZZ,XDZX,1,IKU,1)      ! result at mass point
  ZHGRADGOG(:,:,:,2) =  MXF( MYF(GY_U_UV(XUT,XDYY,XDZZ,XDZY,1,IKU,1) )) ! result at mass point
  ZHGRADGOG(:,:,:,3) =  MYF( MXF(GX_V_UV(XVT,XDXX,XDZZ,XDZX,1,IKU,1) )) ! result at mass point
  ZHGRADGOG(:,:,:,4) =  GY_V_M(XVT(:,:,:), XDYY,XDZZ,XDZY,1,IKU,1)      ! result at mass point
  ! DUDX=PHGRADGOG(:,:,1)
  ! DUDY=PHGRADGOG(:,:,2)
  ! DVDX=PHGRADGOG(:,:,3)
  ! DVDY=PHGRADGOG(:,:,4)
ELSE
  ALLOCATE(ZHGRADGOG(0,0,0,0))
END IF
!
