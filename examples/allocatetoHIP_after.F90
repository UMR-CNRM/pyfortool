...
MNH_HIPALLOCATE(XTHT,IIU,IJU,IKU)     ; XTHT = 0.0
!$acc enter data copyin(XTHT)
MNH_HIPALLOCATE(XRUS,IIU,IJU,IKU)     ; XRUS = 0.0
MNH_HIPALLOCATE(XRVS,IIU,IJU,IKU)     ; XRVS = 0.0
MNH_HIPALLOCATE(XRWS,IIU,IJU,IKU)     ; XRWS = 0.0
!$acc enter data copyin(XRUS,XRVS,XRWS)
MNH_HIPALLOCATE(XRUS_PRES,IIU,IJU,IKU); XRUS_PRES = 0.0
MNH_HIPALLOCATE(XRVS_PRES,IIU,IJU,IKU); XRVS_PRES = 0.0
MNH_HIPALLOCATE(XRWS_PRES,IIU,IJU,IKU); XRWS_PRES = 0.0
!$acc enter data copyin( XRUS_PRES, XRVS_PRES, XRWS_PRES )
MNH_HIPALLOCATE(XRTHS,IIU,IJU,IKU)    ; XRTHS = 0.0
!$acc enter data copyin(XRTHS)
MNH_HIPALLOCATE(XRTHS_CLD,IIU,IJU,IKU); XRTHS_CLD = 0.0
...
END IF
!
MNH_HIPALLOCATE(XPABSM,IIU,IJU,IKU) ; XPABSM = 0.0
MNH_HIPALLOCATE(XPABST,IIU,IJU,IKU) ; XPABST = 0.0
!$acc enter data copyin(XPABST, XPABSM)
!
MNH_HIPALLOCATE(XRT,IIU,IJU,IKU,NRR) ;     XRT = 0.0
!$acc enter data copyin( XRT )
MNH_HIPALLOCATE(XRRS,IIU,IJU,IKU,NRR) ;    XRRS = 0.0
MNH_HIPALLOCATE(XRRS_CLD,IIU,IJU,IKU,NRR); XRRS_CLD = 0.0
...
MNH_HIPALLOCATE(XRSVS,IIU,IJU,IKU,NSV);     XRSVS = 0.
MNH_HIPALLOCATE(XRSVS_CLD,IIU,IJU,IKU,NSV); XRSVS_CLD = 0.0
...
!
MNH_HIPALLOCATE(XDXX,IIU,IJU,IKU)
MNH_HIPALLOCATE(XDYY,IIU,IJU,IKU)
MNH_HIPALLOCATE(XDZX,IIU,IJU,IKU)
MNH_HIPALLOCATE(XDZY,IIU,IJU,IKU)
MNH_HIPALLOCATE(XDZZ,IIU,IJU,IKU)
...
END IF
!
MNH_HIPALLOCATE(XPHIT,IIU,IJU,IKU)
MNH_HIPALLOCATE(XRHODREF,IIU,IJU,IKU)
MNH_HIPALLOCATE(XTHVREF,IIU,IJU,IKU)
MNH_HIPALLOCATE(XEXNREF,IIU,IJU,IKU)
!$acc enter data create( XPHIT, XRHODREF, XTHVREF, XEXNREF )
MNH_HIPALLOCATE(XRHODJ,IIU,IJU,IKU)
!$acc enter data create(XRHODJ)
IF (CEQNSYS=='DUR' .AND. LUSERV) THEN
  MNH_HIPALLOCATE(XRVREF,IIU,IJU,IKU)
ELSE
  MNH_HIPALLOCATE(XRVREF,0,0,0)
END IF
...
IF ( CPRESOPT /= 'ZSOLV' ) THEN
  IF (L2D) THEN
    MNH_HIPALLOCATE(XBFY,IIY,IJY,IKU)
  ELSE
    MNH_HIPALLOCATE(XBFY,IJY,IIY,IKU) ! transposition needed by the optimisation of the
                                ! FFT solver
  END IF
ELSE
  MNH_HIPALLOCATE(XBFY,0,0,0)
END IF

CALL GET_DIM_EXT_ll('B',IIU_B,IJU_B)
IF ( CPRESOPT == 'ZRESI' .OR. CPRESOPT == 'ZSOLV' ) THEN
  MNH_HIPALLOCATE(XBFB,IIU_B,IJU_B,IKU)
ELSE
  MNH_HIPALLOCATE(XBFB,0,0,0)
END IF

IF ( CPRESOPT == 'ZRESI' ) THEN
  CALL GET_DIM_EXTZ_ll('SXP2_YP1_Z',IIU_SXP2_YP1_Z_ll,IJU_SXP2_YP1_Z_ll,IKU_SXP2_YP1_Z_ll)
  MNH_HIPALLOCATE(XBF_SXP2_YP1_Z,IIU_SXP2_YP1_Z_ll,IJU_SXP2_YP1_Z_ll,IKU_SXP2_YP1_Z_ll)
ELSE
  MNH_HIPALLOCATE(XBF_SXP2_YP1_Z,0,0,0)
END IF
IF ( CPRESOPT /= 'ZSOLV' ) THEN
  MNH_HIPALLOCATE(XAF,IKU)
  MNH_HIPALLOCATE(XCF,IKU)
  MNH_HIPALLOCATE(XTRIGSX,3*IIU_ll)
  MNH_HIPALLOCATE(XTRIGSY,3*IJU_ll)
ELSE
  MNH_HIPALLOCATE(XAF,0)
  MNH_HIPALLOCATE(XCF,0)
  MNH_HIPALLOCATE(XTRIGSX,0)
  MNH_HIPALLOCATE(XTRIGSY,0)
END IF
MNH_HIPALLOCATE(XRHOM,IKU)
